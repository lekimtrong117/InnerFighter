...
public class IceHammerSkillControl : SpellSkill
{
    private void Awake()
    {
        SkillSetUp();
    ...
    public override void OnSkillCast(Vector3 cast_point, Vector3 target_point)
    {
        ...
        //
        IceHammer ice_hammer = PoolManager.Instance.Spawn("IceHammer").gameObject.GetComponent<IceHammer>();
        ...
        ice_hammer.SetUp(cast_point, damageData, Target_tag.Enemy,false, false);
        Vector3 dir_to_target = target_point - Yena.Instance.trans.position;
        dir_to_target.y = 0;
        dir_to_target.Normalize();
        ice_hammer.StayAT_IN(Yena.Instance.trans.position+dir_to_target*3, 0.5f);
        ice_hammer.trans.forward = dir_to_target;
        ice_hammer.animator.Play("crush");
    }
}
